import { HydrationBoundary, QueryClient, dehydrate } from '@tanstack/react-query';
import { getMovies, queries } from '@/services/movies';
import ListMoviesFeature from '@/features/list-movie';
import { staticPage } from '@/data/staticPage';
import ParagraphUI from '@/ui/paragraph';
import { LanguageTypes } from '@/types';
import { ParamsTypes } from '@/services/movies/types';

interface PropsTypes {
  params: {
    language: LanguageTypes;
  }
}

const ListPage = async ({ params }: PropsTypes) => {
  const queryClient = new QueryClient();

  const movieParams: ParamsTypes = {
    page: 1,
    size: 10,
  };

  const query = async () => {
    await queryClient.prefetchQuery({
      queryKey: [queries.GET_MOVIES],
      queryFn: () => getMovies(movieParams),
    });
  };

  query();

  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <div className="flex flex-col gap-y-10 min-h-[72vh] container py-10">
        <div>
          <ParagraphUI
            title={staticPage[`${params.language}`].paragraphListPage.title}
            text={staticPage[`${params.language}`].paragraphListPage.description}
          />
        </div>
        <div>
          <ListMoviesFeature language={params.language} />
        </div>
      </div>
    </HydrationBoundary>
  );
};

export default ListPage;
